...
class LoadSimpleCmsData extends ContainerAware implements FixtureInterface
{
    public function load(ObjectManager $dm)
    {
        ...
        $route->setPosition($root, 'static');
        $route->setDefault('_controller', 'AcmeMainBundle:Demo:index');

        $dm->persist($route);

        $this->createMenuItem($dm, $root, 'hardcoded', $route, array('en' => 'Static', 'de' => 'Statisch'));
